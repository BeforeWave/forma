...
This `forma-creator` bundle is fixed to `codex`. Its generated plan-first output must contain exactly five stage skill directories. Direct bundle defaults use the `forma-*` pattern for:
...
- Generate a Codex-ready direct skill bundle root containing the five default `forma-*` stage directories for `plan`, `ground`, `lock`, `execute`, and `showhand`, or the exact `rename.stages` names confirmed by the user.
