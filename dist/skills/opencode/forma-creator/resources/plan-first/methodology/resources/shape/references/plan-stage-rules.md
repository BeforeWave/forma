...
- After the user reviews and confirms the proposal, stop and hand off to the lock stage.
- The handoff should tell the user that the selected grounding producer supplies repository or domain facts when needed, and the lock stage is responsible only for writing confirmed facts into `plan.md` and `tasks.md`.
