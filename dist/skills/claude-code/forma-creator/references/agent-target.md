...
This `forma-creator` bundle is fixed to `claude-code`. Its generated plan-first output must contain exactly five stage skill directories. Direct bundle defaults use the `forma-*` pattern for:
...
- Generate a Claude Code-ready direct skill bundle root containing the five default `forma-*` stage directories for `plan`, `ground`, `lock`, `execute`, and `showhand`, or the exact `rename.stages` names confirmed by the user.
...
- For plugin output, strip the exact plugin-name prefix from generated skill names when present. For plugin `forma`, generated direct skill names using the `forma-*` pattern become plugin-local `plan`, `ground`, `lock`, `execute`, and `showhand` stages.
