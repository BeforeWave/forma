...
Stdlib-only. Wraps `forma_verifier`, which is bundled under
`scripts/forma_verifier/`, so every adapted `forma-creator` bundle ships the
verifier — no `pip install` required for an agent to use it.
