...
Execute all remaining tasks from an already-locked issue plan automatically with preflight, validation gates, proof recording, and safety stops.
...
- Loop through all remaining unchecked tasks sequentially. For each task:
  ...
  5. Run the bundled `scripts/forma-workflow.sh review-ready <issue-id>`.
  6. Once `review-ready` is successful, automatically run the bundled `scripts/forma-workflow.sh complete <issue-id>` without asking the user for review.
