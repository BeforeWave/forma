...
如果这套规则要团队共用或长期维护，再把 profile 放进版本控制。已经有 review 过的 profile 时，可以直接用 `forma build bundle --profile <profile.yaml>` 或 `forma build plugin --profile <profile.yaml>` 生成 workflow 产物。完整命令见 [Quick Start](./docs/quick-start.zh-CN.md) 和 [Usage](./docs/usage.zh-CN.md)。
