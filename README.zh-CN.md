...
**把项目规则变成专属 agent workflow。**

Superpowers + AGENTS.md 给 agent 规则和流程。对轻量项目，这个组合够了。

Forma 把项目规则变成一组 workflow skills，让 agent 在实现前先预览规则如何落到当前任务，再按 task contract 执行并留下 proof。
...
Forma 的区别是：项目规则会进入 workflow，让 agent 写出不同的计划重点。
这些偏向不用你每次提醒 agent。
...
| 名称 | 作用 |
|---|---|
| `profile` | 从项目里提炼出的工程规则说明，写清阶段约束、工具习惯、验证、proof 和停手条件。 |
| `workflow bundle / plugin` | 按 profile 生成，安装到 Codex、Claude Code 或 OpenCode，让 agent 按同一套规则工作。 |
| `task contract` | agent 每次任务开始前写下的计划边界，记录目标、范围、验证和 proof。 |

profile 要长期复用时再提交进版本控制。只想试一套 workflow 时，它可以是临时文件，生成后不用保留。
...
团队后续调整 profile，就等于调整 workflow 守护哪些准则。
...
最轻入口是让 agent 先整理项目规则，确认后生成并安装 workflow：
...
agent 会自己读取 Forma 的 profile 编写指南，并在你确认规则后生成、验证和安装 workflow。
...
如果这套规则要团队共用或长期维护，再把 profile 放进版本控制。已经有 review 过的 profile 时，可以直接用 `forma create-bundle --profile <profile.yaml>` 或 `forma create-plugin --profile <profile.yaml>` 生成 workflow 产物。完整命令见 [Quick Start](./docs/quick-start.zh-CN.md) 和 [Usage](./docs/usage.zh-CN.md)。
...
| | |
|---|---|
| [Quick Start](./docs/quick-start.zh-CN.md) | 首次运行、生成、验证和安装 workflow。 |
