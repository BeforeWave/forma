...
Forma 把项目规则编译成专属任务工作流。每个任务里，生成 workflow 会把规则落成具体边界、验证步骤和 proof。

AI coding 是第一个落点，因为代码任务有清楚的来源、测试、diff 和 review proof。同一个模型也可以用于其他有来源、边界、验证和 proof 的重复工作。

Forma 和固定 workflow 工具不同：它不是让所有项目安装同一套流程，而是从项目或任务自己的规则生成 workflow。
