...
- Do not run `scripts/issue-workflow.sh init <issue-id>`, do not write `plan.md`, do not write `tasks.md`, and do not commit plan files from `showhand`.
- Loop through all remaining unchecked tasks sequentially. For each task:
  1. Read `scripts/issue-workflow.sh next <issue-id>` to get the source of truth for the current task.
  ...
  5. Run `scripts/issue-workflow.sh review-ready <issue-id>`.
  6. Once `review-ready` is successful, automatically run `scripts/issue-workflow.sh complete <issue-id>` without asking the user for review.
