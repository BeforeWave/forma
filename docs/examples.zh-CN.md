...
| generated baseline | profile 编译后变成什么 skill bundle，skill 名字、references、scripts 和 manifest 如何落盘。 |
...
backend sample 已提交生成后的 Codex 和 Claude Code baseline：
...
它们用于检查 compiler drift，也可以直接看 profile 编译后的结构：
...
| `plans/issue-bundle-plugin-install-surface/` | `create-bundle` / `create-plugin` / `install` release surface、dist 产物和 docs 更新。 |
...
- docs、generated-baseline、migration、governance、cross-layer 等重规则进 conditional overlays；
...
- generated baseline 不是 runtime 成功证明；它证明 compiler 输出结构和 drift。
