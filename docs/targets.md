...
The plugin skill ids are the emitted skill names recorded in
`.forma-manifest.json`, so renamed profile or creator skills stay visible
...
- Codex plugin `plugin.json` uses the profile `bundle.name` or creator
  `rename.prefix` as plugin id, and its `skills` entries must match the emitted
  nested skills.
