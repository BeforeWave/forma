...
这个顶层 profile 引入了 sample base、development、backend 和 Go 层，然后把生成的阶段技能重命名成后端 issue workflow 的叫法。
...
Includes 适合组织默认规则、开发政策、后端政策、语言政策等稳定层。不要用 includes 隐藏一次性用户指令；这种情况应该用 temporary injection。
...
| `constraints.gauge` | 只读取证和证据整理。 |
| `constraints.seal` | 计划定稿、已接受任务、验证契约和交接说明。 |
| `constraints.pour` | 当前任务执行、评审门禁和证明。 |
