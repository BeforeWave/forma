...
Profile 描述 Forma 如何把标准方法专门化到某个项目、团队、路线、语言或目标命名约定。它是严格 YAML：未知顶层键或未知嵌套键会被拒绝，避免错误静默改变 workflow contract。
...
这个 top-level profile include 了 sample base、development、backend 和 Go 层，然后把生成阶段技能重命名成后端 issue workflow 的叫法。
...
Included profiles 会先于当前 profile 解析。后面的 profile 可以按 Forma 的 merge rules 细化前面的字段。

Includes 适合组织默认规则、开发政策、后端政策、语言政策等稳定层。不要用 includes 隐藏一次性用户指令；这种情况用 temporary injection。
...
| `constraints.gauge` | 只读 grounding 和证据收集。 |
| `constraints.seal` | 计划定稿、已接受任务、验证契约和 handoff certification。 |
| `constraints.pour` | 当前任务执行、review gate 和证明。 |
...
Resources 适合稳定 references 和显式 helpers。来源读取器和辅助脚本应该由 profile 或 temporary injection 明确选择，不应该作为无归属的基础能力出现。
...
Validation commands 是 workflow 指引，不会替代 sealed plan 中的任务专用验证。
...
- 把 `forma verify` 当成人工 profile review 的替代品。

## 相关文档

- [Workflow Contract](./workflow-contract.zh-CN.md)：profile 专门化的 contract。
