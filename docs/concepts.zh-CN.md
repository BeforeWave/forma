...
Forma 在项目说明和具体实现之间加了一层工作流。Agent 会先把项目规则应用到当前任务，说明能改什么、怎么检查结果，再按这个计划去修改。
...
| 阶段 | 作用 | 边界 |
|---|---|---|
| `plan` | 按项目准则检查目标，形成 proposal。 | 不写文件，不执行任务。 |
...
| 路径 | 适合 | 结果 |
|---|---|---|
| `forma explain agent` -> profile authoring | 从项目规则生成 workflow；profile 可临时，也可长期维护。 | profile + 已验证 workflow 产物。 |
